    ...
    <div className="w-full h-36 border-b-[1px] border-b-gray-800">
        <div className="max-w-screen-xl h-full mx-auto flex items-center justify-between">
            <div className="flex items-center gap-20">
                <div>
                    <img className="h-16 w-16 mx-auto flex items-center gap-20 rounded-sm group hover:text-black hover:underline underline-offset-2 decoration-[1px] cursor-pointer duration-500 shadow-md" 
                        src={loginPerson} alt="userLogo"
                        style={{ transition: 'all 0.3s' }} onMouseOver={(e) => { e.target.style.opacity = 0.8; e.target.style.boxShadow = '0 2px 6px rgba(0, 0, 0, 0.2)';}}
                                                            onMouseOut={(e) => { e.target.style.opacity = 1; e.target.style.boxShadow = 'none';}}
                    />
                </div>
                <div className="relative">
                    <img className="logo w-18 h-16 mx-auto flex gap-20 rounded-sm group hover:text-black hover:underline underline-offset-2 decoration-[1px] cursor-pointer duration-500 shadow-md" 
                        src={cart} alt="cartImg" 
                        style={{ transition: 'all 0.3s' }} onMouseOver={(e) => { e.target.style.opacity = 0.8; e.target.style.boxShadow = '0 2px 6px rgba(0, 0, 0, 0.2)';}}
                                                            onMouseOut={(e) => { e.target.style.opacity = 1; e.target.style.boxShadow = 'none';}}
                    />
            ...
            <div className="ml-auto">
                <img className="logo w-50 h-36 mx-auto flex" src={logo} alt="logo" />
            </div>
        </div>
    </div>
  )
